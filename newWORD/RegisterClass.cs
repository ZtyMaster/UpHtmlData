 ...
 namespace newWORD
{
    ...
    public  class RegisterClass
    {
        ...
        public string GetCodeMD5(string text)
        {
        ...
        public bool RegistIt(string currentCode, string realCode)
        {
            if (realCode != "")
            {
                if (currentCode == GetMd5(GetMd5(realCode))) 
                {
        ...
        public bool BoolRegist(string sn)
        {
            ...
            try
            {
                keynames = localRegKey.OpenSubKey("software\\StupidsCat\\StupidsCat.ini\\" + GetMd5(GetMd5(sn))).GetValueNames();
                foreach (string name in keynames)
                {
                    if (name == "StupidsCat")
                    {
                        if (localRegKey.OpenSubKey("software\\StupidsCat\\StupidsCat.ini\\" + GetMd5(GetMd5(sn))).GetValue("StupidsCat").ToString() == "BBC6D58D0953F027760A046D58D52786")
                            flag = true;
                    }
                }
                keynames = userRegKey.OpenSubKey("software\\StupidsCat\\StupidsCat.ini\\" + GetMd5( GetMd5(sn))).GetValueNames();
                foreach (string name in keynames)
                {
                    if (name == "StupidsCat")
                    {
                        if (flag && userRegKey.OpenSubKey("software\\StupidsCat\\StupidsCat.ini\\" + GetMd5(GetMd5(sn))).GetValue("StupidsCat").ToString() == "BBC6D58D0953F027760A046D58D52786")
                            return true;
                    }
                }
                return false;
        ...
        {
            string path = text.ToString()+"ztykf";
